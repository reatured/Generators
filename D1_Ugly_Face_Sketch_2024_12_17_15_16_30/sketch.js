let canvasSize = 400; 
let saveImage = true;
let saveImageAmount = 20*15;
let debugHandleLine = false; 
...
function setup() {
  ...
  slider = createSlider(3, 10, defaultThickness);
...
function mousePressed() {
  if (mouseX < 400 && mouseY < 400) {
    if(saveImage){
      for(let k = 0; k < saveImageAmount; k++){
        save(saveFileName +'.png');  // Save the canvas as an image
        reFresh();  // Call your refresh function
      }
    }
    else{
      reFresh(); 
...
function getTimestamp() {
  let now = new Date();
  let month = String(now.getMonth() + 1).padStart(2, "0"); // Month (0-11)
  let day = String(now.getDate()).padStart(2, "0"); // Day of the month
  let hours = String(now.getHours()).padStart(2, "0"); // Hours (0-23)
  let minutes = String(now.getMinutes()).padStart(2, "0"); // Minutes (0-59)
  let seconds = String(now.getSeconds()).padStart(2, "0"); // Seconds (0-59)

  return month + day + hours + minutes + seconds;
}

let saveFileName = "SketchedFace_" + getTimestamp(); // Use the formatted timestamp